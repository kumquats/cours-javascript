...
}).then(displayNews); // A.4. Appeler une API REST/JSON en GET
...
fetch('http://localhost:8080/api/v1/pizzas').then(function (response) {
  return response.json();
}).then(renderHome) // B.3.6. retirer la classe is-loading
.then(function () {
  return jquery__WEBPACK_IMPORTED_MODULE_3___default()('.pizzasContainer').removeClass('is-loading');
}) // B.4.2. détecter le clic sur les liens de la home
.then(function (pizzasContainer) {
  return pizzasContainer.find('a').click(function (event) {
    event.preventDefault();
    window.open(jquery__WEBPACK_IMPORTED_MODULE_3___default()(event.currentTarget).attr('href'), 'popup', 'width=350,height=200,menubar=0,toolbar=0,location=0,personalbar=0,status=0');
  });
}); // B.2. La fonction $()
...
jquery__WEBPACK_IMPORTED_MODULE_3___default()('.pizzasContainer').addClass('is-loading'); // B.4.1. auc clic sur le logo on affiche la homepage

jquery__WEBPACK_IMPORTED_MODULE_3___default()('a.navbar-brand').on('click', function (event) {
  return renderHome();
});
